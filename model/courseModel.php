...
function updateCourseById(
    ...
    $db = connectionDb();
    $checkUpdate = false;
    $sql = "UPDATE `course` SET `name` = :nameCourse, `slug` = :slug, `status` = :statusCourse, `department_id` = :idDepartment,`description` = :descriptionCourse,`updated_at` = :updated_at WHERE `id` = :id AND `deleted_at` IS NULL";
...
function getAllDataCourses($keyword = null)
{
    $db = connectionDb();
    $key = "%{$keyword}%";
    $sql = "SELECT departments.name AS department_name, course.*
            FROM departments
            INNER JOIN course ON departments.id = course.department_id 
            WHERE (course.`name` LIKE :keywordName OR course.`slug` LIKE :keywordSlug OR departments.`name` LIKE :keywordNameDepartment) 
...
function insertCourse(
    ...
    $db = connectionDb();
    $flagInsert = false;
    $sqlInsert = "INSERT INTO `course`(`name`, `slug`, `status`, `description`, `department_id`, `created_at`) VALUES(:nameCourse, :slug, :statusCourse, :descriptionCourse, :idDepartment, :created_at)";
